...
def parse_csv(path):
    ...
    for i, l in enumerate(lines):
        if l.startswith("# DATE") or (l.startswith("#") and "MODE" not in l and meta["date"] is None):
        ...
        if ("Cp [F]" in l) or ("Current Density" in l) or ("Voltage [V]" in l and "Current [A]" in l):
            header = [c.strip() for c in l.split(",")]
            header_idx = i
            break
    if header is None:
        raise ValueError(f"컬럼 헤더를 찾지 못함: {path}")

    # 모드 자동 판별
    if "Cp [F]" in header:
        mode = "CV"
    elif "Current [A]" in header or "Current Density [A/cm2]" in header:
        mode = "IV"
    else:
        raise ValueError(f"CV/IV 판별 실패: {header}")
    ...
    for l in lines[header_idx + 1:]:
        ...
        if len(p) < len(header):
            continue
        if p[0].startswith("Append") or p[1].strip() == "":
            continue
        try:
    ...
    return {"mode": mode, "meta": meta, "col": col, "rows": rows}
...
def analyze_full(path_or_list, thickness_nm, radius_um, structure="MIM", material=None):
    ...
    if parsed[0]["mode"] == "CV":
        ...
        V, Cp, G, freq = [], [], [], None
        for r in p["rows"]:
            v = r[c["DC VOLT [V]"]]; cp = r[c["Cp [F]"]]
            if v is None or cp is None:
                continue
            g = r[c["G [S]"]] if "G [S]" in c else None
            freq = r[c["Frequency [Hz]"]] if "Frequency [Hz]" in c else None
